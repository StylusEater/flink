...
import org.apache.flink.javascript.api.JavaScriptPlanBinder;
import static org.apache.flink.javascript.api.JavaScriptPlanBinder.DEBUG;
import static org.apache.flink.javascript.api.JavaScriptPlanBinder.FLINK_JAVASCRIPT_BINARY_PATH;
import static org.apache.flink.javascript.api.JavaScriptPlanBinder.FLINK_JAVASCRIPT_DC_ID;
import static org.apache.flink.javascript.api.JavaScriptPlanBinder.FLINK_JAVASCRIPT_PLAN_NAME;
import static org.apache.flink.javascript.api.JavaScriptPlanBinder.FLINK_TMP_DATA_DIR;
import static org.apache.flink.javascript.api.JavaScriptPlanBinder.PLANBINDER_CONFIG_BCVAR_COUNT;
import static org.apache.flink.javascript.api.JavaScriptPlanBinder.PLANBINDER_CONFIG_BCVAR_NAME_PREFIX;
import org.apache.flink.util.Collector;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * This streamer is used by functions to send/receive data to/from an external javascript process.
 */
public class JavaScriptStreamer implements Serializable {
	protected static final Logger LOG = LoggerFactory.getLogger(JavaScriptStreamer.class);
	...
	public JavaScriptStreamer(AbstractRichFunction function, int id) {
		this.id = id;
		this.debug = DEBUG;
		planArguments = JavaScriptPlanBinder.arguments.toString();
	...
	private void startJavaScript() throws IOException {
		...
		String path = function.getRuntimeContext().getDistributedCache().getFile(FLINK_JAVASCRIPT_DC_ID).getAbsolutePath();
		String planPath = path + FLINK_JAVASCRIPT_PLAN_NAME;
		...
		if (debug) {
			...
			LOG.info("Waiting for JavaScript Process : " + function.getRuntimeContext().getTaskName()
					+ " Run javascript " + planPath + planArguments);
		} else {
			process = Runtime.getRuntime().exec(javascriptBinaryPath + " -O -B " + planPath + planArguments);
